...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Редактирование записи №{{$reward_credit->id }}</h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
            @if (session('success'))
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="card card-primary">
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form action="{{ route('reward_credits.update', $reward_credit->id) }} " method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="description">Описание</label>
                                    <textarea name="description" class="tiny" cols="80" rows="5">{{old('description') ?? $reward_credit->description}}</textarea>
                                    @error('description')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="advanced_description" class="tiny" cols="80" rows="5">{{old('advanced_description') ?? $reward_credit->advanced_description}}</textarea>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea type="text" class="form-control tiny" id="email-bank" name="mail_for_demands">{{old('mail_for_demands') ?? $reward_credit->mail_for_demands}}</textarea>
                                    @error('mail_for_demands')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputPassword1">Куратор в банке</label>
                                    <textarea type="text" class="form-control tiny" id="email-copy-bank" name="curator" >{{old('curator') ?? $reward_credit->curator}}</textarea>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputPassword1">Личный кабинет</label>
                                    <textarea type="text" class="form-control tiny" id="contact-bank" name="lk">{{old('lk') ?? $reward_credit->lk}}</textarea>
                                </div>

                                <div class="form-group">
                                    <label for="exampleInputPassword1">Контакты банка по заявке</label>
                                    <textarea type="text" class="form-control tiny" id="alt-contact-bank" name="bank_contacts">{{old('bank_contacts') ?? $reward_credit->bank_contacts}}</textarea>
                                </div>
                                <div class="form-group">
                                    <input type="checkbox" name="only_text" @if($reward_credit->only_text == 1) checked @endif >
                                    <label for="checkboxBank">
                                        Только текст
                                    </label>
                                    <textarea name="text" class="tiny" cols="80" rows="5">{{old('text') ?? $reward_credit->text}}</textarea>
                                </div>
                                <div class="form-group">
