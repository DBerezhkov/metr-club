...
@section('title', 'Заявки на кредиты')
...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Заявки на потребы</h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
                    ...
                    <div class="card-header border-0 py-1">
                        <div class="form-row">
                            @if(auth()->user()->hasRole('supervisor'))
                                <div class="col-auto">
                                    <form action="{{route('supervisorCreditsExport')}}">
                        ...
                        <div id="collapseTwo"
                            ...
                            <div class="card-body py-2">
                                <form @if(auth()->user()->hasRole('supervisor')) action="{{route('supervisor_credits.index')}}" @else action="{{route('credits.index')}}" @endif>
                                    <div class="form-row mb-3">
                                        ...
                                        <div class="col-auto" style="padding-top: 31px">
                                            @if(isset($_GET) && !empty($_GET))
                                                @if(auth()->user()->hasRole('supervisor'))
                                                    <a href="{{route('supervisor_credits.index')}}" class="btn btn-success">Сбросить</a>
                                                @else
                                                    <a href="{{route('credits.index')}}" class="btn btn-success">Сбросить</a>
                                                @endif
                                            @endif
                                        </div>
                                    </div>
                                    @if(isset($_GET) && !empty($_GET))
                                        <div><h6><b>Найдено записей: {{$credits_count}} </b></h6></div>
                                    @endif
                                </form>
                            </div>
                        </div>
            ...
            <div class="col-md-12">
                <div class="card card-primary">
                    ...
                    <div class="card-body p-0">
                        <table class="table table-striped projects">
                            ...
                            @foreach($credits as $credit)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('credits.show', $credit->id)}}">{{ $credit['name'] }}</a>
                                    </td>
                                    <td>
                                        {{ $credit['phone'] }}
                                    </td>
                                    ...
                                    <td>
                                        @if($credit->agent)
                                            <a href="{{route('employees.show', $credit->agent_id)}}"> {{ $credit->agent['name'] }}</a>
                                        @elseif($credit->agent()->withTrashed()->get()->first() != null)
                                            <div>{{$credit->agent()->withTrashed()->get()->first()->name}}</div>
                                        @endif
                                    </td>
                                    @endif
                                    <td>
                                    ...
                                    <td>
                                        {{$credit -> printBanksList($credit->banks_list)}}
                                    </td>
                                    <td class="project-actions text-center">
                                        <a class="btn btn-primary btn-sm mb-2" href="{{route('credits.show', $credit->id)}}">
                                            Открыть
                                        </a>
                                    </td>
                                </tr>
                        ...
                        <div class="ml-3 my-3">
                            {{ $credits->links('vendor.pagination.bootstrap-4') }}
                        </div>
                    </div>
                </div>
            </div>
